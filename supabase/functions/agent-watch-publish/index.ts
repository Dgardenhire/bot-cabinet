...
Deno.serve(async (request) => {
  ...
  const expectedSecret = Deno.env.get("AGENT_WATCH_PUBLISH_SECRET") ?? "";
  ...
  const currentResponse = await fetch(`${supabaseUrl}/rest/v1/agent_watch_publications?slug=eq.${encodeURIComponent(publication.item.slug)}&select=revision&order=revision.desc&limit=1`, { headers: authHeaders });
  ...
  if (currentRevision !== publication.expectedRevision) return json(409, { error: "The public item changed after review", currentRevision });

  const nextRevision = currentRevision + 1;
  const insertResponse = await fetch(`${supabaseUrl}/rest/v1/agent_watch_publications`, {
    method: "POST",
    headers: { ...authHeaders, Prefer: "return=minimal" },
    body: JSON.stringify({
      slug: publication.item.slug,
  ...
  if (insertResponse.status === 409) return json(409, { error: "The public item changed after review" });
  if (!insertResponse.ok) return json(503, { error: "Publication unavailable" });
  return json(201, { published: true, slug: publication.item.slug, revision: nextRevision });
});
