...
export async function readBlueprintRequestBody(
  ...
  while (true) {
    ...
    if (totalBytes > BOT_BLUEPRINT_MAX_REQUEST_BYTES) {
      await reader.cancel();
      return null;
    }
    chunks.push(value);
...
export function getClientIp(request: Request): string | null {
  // The hosted Supabase gateway supplies this Cloudflare header. Fail closed
  // when it is absent instead of trusting caller-controlled forwarding chains.
