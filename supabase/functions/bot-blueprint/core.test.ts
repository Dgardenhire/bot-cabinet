...
Deno.test("the request reader stops an oversized streamed body without a content-length header", async () => {
  const chunk = new TextEncoder().encode("x".repeat(8_500));
  let cancelled = false;
  let reads = 0;
...
Deno.test("the request reader rejects compressed request bodies", async () => {
  const request = blueprintRequest();
  request.headers.set("content-encoding", "gzip");
  assert.equal(await readBlueprintRequestBody(request), null);
});

Deno.test("request identity uses only the gateway-supplied client address", () => {
  const spoofedForwardingRequest = new Request("https://function.example", {
    headers: {
      "x-forwarded-for": "203.0.113.99",
      "x-real-ip": "203.0.113.99",
    },
  });
  assert.equal(getClientIp(spoofedForwardingRequest), null);

  const gatewayRequest = new Request("https://function.example", {
    headers: { "cf-connecting-ip": "203.0.113.10" },
  });
  assert.equal(getClientIp(gatewayRequest), "203.0.113.10");
});
